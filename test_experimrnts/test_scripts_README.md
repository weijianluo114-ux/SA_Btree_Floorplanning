...
1. **自动编译**  
   通过 `--skip_make` 控制是否执行 `make -f Makefile.debug`，默认开启编译。

2. **灵活的参数传递**  
   所有 Bash 脚本中的参数（可执行文件路径、测试数据、空白比例、运行次数、输出目录、日志文件、种子文件等）均可通过命令行指定，并提供合理默认值。

3. **种子生成**  
   采用与 Bash 相同的 `((RANDOM<<15)+RANDOM)` 方法生成随机种子，范围 0 ~ 2^30-1，并保存到种子文件。
